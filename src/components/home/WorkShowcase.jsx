...
export default function WorkShowcase() {
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            gsap.fromTo(sectionRef.current.querySelector('.ws-header'),
                { y: 40, opacity: 0 },
                { y: 0, opacity: 1, duration: 0.9, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' } })
            gsap.fromTo(cardRefs.current,
                { y: 60, opacity: 0 },
                { y: 0, opacity: 1, stagger: 0.12, duration: 0.9, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 65%' } })
            cardRefs.current.forEach((card) => {
                if (!card) return
                gsap.to(card.querySelector('.ws-img'), {
        ...
        <section ref={sectionRef} className="py-24 md:py-36 px-6 md:px-14 bg-t-bg">
            <div className="max-w-[1400px] mx-auto">
                <div className="ws-header flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-14">
                    <div>
                        <div className="flex items-center gap-3 mb-4"><span className="w-7 h-px accent-line" /><span className="text-t-muted text-[10px] tracking-[0.3em] uppercase">Our Work</span></div>
                    ...
                    <Link to="/case-studies" className="shrink-0 inline-flex items-center gap-2 text-t-secondary text-sm hover:text-t-primary transition-colors">
                        View all projects
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {PROJECTS.map((p, i) => (
                        <a key={i} ref={(el) => { cardRefs.current[i] = el }}
                            ...
                            <div className="relative overflow-hidden h-60 md:h-72 bg-t-surface-2">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <span className="flex items-center gap-2 bg-white text-black text-xs font-bold px-4 py-2 rounded-full translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                        Visit Site
                                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 8L8 2M8 2H3M8 2v5" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                                    </span>
                                </div>
                            </div>
                            <div className="p-6 flex items-center justify-between bg-t-surface">
                                <div>
                                    <h3 className="text-t-primary font-semibold text-base mb-1 group-hover:text-t-accent transition-colors duration-300">{p.name}</h3>
                                    <span className="text-t-muted text-xs">{p.tag}</span>
                                </div>
                                <svg className="text-t-muted group-hover:text-t-accent transition-colors duration-300" width="18" height="18" viewBox="0 0 18 18" fill="none">
